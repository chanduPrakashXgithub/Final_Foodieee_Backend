...
const getAllvendors=async(req,res)=>{
    try {
        const allvendors=await vendors.find().populate('firm');
...
const getvendorsById=async(req,res)=>{
    ...
    try {
        const vendor=await vendors.findById(vendorsId);
        if(!vendor){
            return res.status(404).json({message:"Vendor not found"})
        }
        const vendorsFirmId=vendor.firm[0]._id;
        res.status(200).json({vendor, vendorsFirmId});
    }catch(err){
        console.error("Error during login:", err);
        res.status(500).json({ message: "Something went wrong", error: err.message });
